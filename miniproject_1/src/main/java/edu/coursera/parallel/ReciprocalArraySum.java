...
//        }
        ...
        ReciprocalArraySumTask temp = new ReciprocalArraySumTask(0, input.length, input);
        ForkJoinPool.commonPool().invoke(temp);
        sum = temp.getValue();
    ...
    protected static double parManyTaskArraySum(final double[] input,
                                                final int numTasks) {
        double sum = 0;
        ReciprocalArraySumTask[] tasksArray = new ReciprocalArraySumTask[numTasks];
        for (int i = 0; i < numTasks; ++i) {
            tasksArray[i] = new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length),
                    getChunkEndExclusive(i, numTasks, input.length),
                    input);
            //tasksArray[i].fork();
            ForkJoinPool.commonPool().invoke(tasksArray[i]);
            sum += tasksArray[i].getValue();
        }
